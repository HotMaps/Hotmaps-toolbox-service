...
def vector_query(scalevalue,vector_table_requested, geometry,toCRS,where_string,select_string):
    """
    ...
    if scalevalue == 'hectare':
        return vector_query_hectares(vector_table_requested, geometry,toCRS,where_string,select_string)
    elif scalevalue == 'nuts':
        id_selected_list = helper.adapt_nuts_list(geometry)
        return vector_query_nuts(vector_table_requested, id_selected_list,where_string,select_string)
    elif scalevalue == 'lau':
        id_selected_list = helper.adapt_nuts_list(geometry)
        return vector_query_lau(vector_table_requested, id_selected_list,toCRS,where_string,select_string)
    return None



def vector_query_hectares(vector_table_requested, geometry,toCRS,where_string,select_string):
    """
    ...
    query= "with subAreas as ( SELECT nuts.nuts_id,nuts.gid FROM geo.nuts " + \
           ...
           "select "+select_string+" from " + vector_table_requested + "subAreas " + \
           "where " + where_string
    print ('query ',query)
...
def vector_query_nuts(vector_table_requested_fild, area_selected,where_string,select_string):
    """
    ...
    :return:
    """
    vector_table_requested_fild = str(vector_table_requested_fild)
    query= "with selected_zone as ( SELECT geom as geom from geo.nuts where nuts_id IN("+ area_selected+") " \
                                                                                                        ...
                                                                                                        "select "+ select_string+"from " + vector_table_requested_fild + "subAreas where " + where_string
    print("vector_query_nuts", query)                                                                                                                                      
    return query



def vector_query_lau(vector_table_requested, area_selected,toCRS,where_string,select_string):
    """
    ...
    query= "with selected_zone as ( SELECT geom" \
                                                       " from public.tbl_lau1_2 where comm_id IN("+ area_selected+")  )," \
                                                                                                        ...
                                                                                                        "AND geo.nuts.year = to_date('2013', 'YYYY') ) select "+select_string +" from " + vector_table_requested + "subAreas where " + where_string
